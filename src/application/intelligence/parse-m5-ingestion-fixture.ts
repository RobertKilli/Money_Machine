...
export function parseM5IngestionFixture(input: unknown, options: { readonly request: IngestionRequest; readonly attempt: IngestionAttempt; readonly recordedAt: string; readonly retrievedAt: string }): M5IngestionFixtureParseResult {
  ...
  root.records.forEach(value => {
    ...
    observations.push(createSourceObservation({ ingestionAttemptId: options.attempt.ingestionAttemptId, sourceArtifactId: artifact.sourceArtifactId, responsePageOrdinal: pageMetadata.pageOrdinal, itemOrdinal, retrievedAt: options.retrievedAt, metadata: pageMetadata, recordedAt: options.recordedAt }));
  });
