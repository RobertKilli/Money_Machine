...
function projectPageToPackageRecord(page: M5ParsedHolderPage, request: M5HolderSnapshotRequestPlan, recordedAt: string): Obj {
  ...
  const envelope = { adapterContractVersion: M5_HOLDER_SNAPSHOT_ADAPTER_VERSION, parserContractVersion: M5_HOLDER_PAGE_FIXTURE_VERSION, providerId: page.providerId, datasetId: page.datasetId, datasetVersion: page.datasetVersion, providerSourceNamespace: page.providerSourceNamespace, chainId: page.chainId, contractAddress: page.contractAddress, observationType: "HOLDER_SNAPSHOT_PAGE", pageOrdinal: page.pageOrdinal, isFinal: page.isFinal, snapshotBlockNumber: page.snapshotBlockNumber.toString(), snapshotBlockHash: page.snapshotBlockHash, snapshotBlockTimestamp: page.snapshotBlockTimestamp, decimals: page.tokenDecimals, totalSupplyAtoms: page.totalSupplyAtoms.toString(), declaredHolderCount: page.declaredHolderCount, declaredPageCount: page.declaredPageCount, holderItems, finality: { referenceBlockNumber: page.finality.referenceBlockNumber.toString(), referenceBlockHash: page.finality.referenceBlockHash, observedAt: page.finality.observedAt, receivedAt: page.finality.receivedAt } };
  return { providerExternalRecordId: page.sourceRecordId, payloadFingerprint: page.payloadFingerprint, pageOrdinal: page.pageOrdinal, itemOrdinal: 0, retrievedAt: page.receipt.receivedAt, recordedAt, observedAt: page.snapshotBlockTimestamp, normalizedEnvelope: envelope, selectedAuditableFields: { metric: "HOLDER_SNAPSHOT_PAGE", pageOrdinal: page.pageOrdinal, receiptAt: page.receipt.receivedAt, effectiveFinalityReceiptAt: page.finality.receivedAt }, metadata: { pageOrdinal: page.pageOrdinal, cursorSafety: "NONE" } };
}
...
export function projectM5HolderPageSetToNormalizedPackage(input: Readonly<{ pageSet: Extract<M5HolderPageSetResult, { status: "COMPLETE" }>; idempotencyKey: string; requestedAt: string; startedAt: string; recordedAt: string }>): ManualNormalizedSourcePackage {
  const records = input.pageSet.pages.map(page => projectPageToPackageRecord(page, input.pageSet.request, input.recordedAt));
