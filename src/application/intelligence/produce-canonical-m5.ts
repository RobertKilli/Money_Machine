...
export async function produceCanonicalM5(input: ProduceCanonicalM5Input, dependencies: M5ProducerDependencies): Promise<CanonicalM5ProducerResult> {
  ...
  if (assessment.coverageStatus !== "COMPLETE" || !assessment.ruleSetAuthorityId || !assessment.coverageAuthorityId || !assessment.coverageFingerprint || assessment.evaluatedRuleCount !== assessment.coveredRuleIds.length) return assessmentIncomplete("M5_ASSEMBLY_SUSPICIOUS_COVERAGE_AUTHORITY_MISSING");
