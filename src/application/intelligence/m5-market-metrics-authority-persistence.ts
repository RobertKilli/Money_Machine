  ...
  return input.unitOfWork.withTransaction(async repositories => {
    ...
    try {
      ...
      const bound = input.materials.map(material => {
        ...
        if (!authority || !member || member.sourceLineageId !== input.sourceLineageId || member.sourceEnvelopeId !== material.sourceEnvelopeId || member.sourceObservationId !== material.sourceObservationId || authority.envelope.sourceEnvelopeId !== material.sourceEnvelopeId || authority.observation.sourceObservationId !== material.sourceObservationId || authority.artifact.providerExternalRecordId !== material.providerExternalRecordId || authority.artifact.payloadFingerprint !== material.payloadFingerprint || authority.envelope.payloadFingerprint !== material.payloadFingerprint) throw new Error("M5_MARKET_AUTHORITY_MATERIAL_MISMATCH");
        ...
        if (metricKind === "LIQUIDITY") {
          ...
          return { ...withWindow, coverageVersion: envelopeText(fields, "coverageVersion"), componentIds: [componentA.id, componentB.id], components: [componentA, componentB] };
        }
        return withWindow;
      });
