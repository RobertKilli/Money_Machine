...
type MarketLiquidityComponent = Readonly<{ id: string; valueAtoms: bigint; scale: number; quoteCurrency: string }>;
...
function validateMaterial(m: MarketSourceMaterial, expected: MarketMetricKind, asOf: string, currency: string): void {
  if (m.metricKind !== expected || !text(m.sourceArtifactId) || !text(m.sourceEnvelopeId) || !text(m.sourceObservationId) || !text(m.providerExternalRecordId) || !SHA.test(m.payloadFingerprint) || !atoms(m.valueAtoms) || m.valueAtoms > UINT256_MAX || !Number.isSafeInteger(m.scale) || m.scale < 0 || m.scale > 36 || !text(m.quoteCurrency) || m.quoteCurrency !== currency || !validTime(m.observedAt) || !validTime(m.availableAt) || m.observedAt > m.availableAt || m.availableAt > asOf) throw new Error("M5_MARKET_MATERIAL_INVALID");
  ...
  if (expected === "VOLUME" && (m.basis !== "ROLLING_24H_REPORTED" || !validTime(m.windowStart) || !validTime(m.windowEnd) || m.windowEnd! > asOf || m.windowStart! > m.windowEnd! || Date.parse(m.windowEnd!) - Date.parse(m.windowStart!) !== 86_400_000)) throw new Error("M5_MARKET_VOLUME_WINDOW_INVALID");
  const expectedComponentCount = m.expectedComponentCount;
  if (expected === "LIQUIDITY" && (m.basis !== "COMPLETE_LIQUIDITY_UNIVERSE" || m.coverageComplete !== true || expectedComponentCount === undefined || !Number.isSafeInteger(expectedComponentCount) || expectedComponentCount < 2 || !m.componentIds || !m.components || m.componentIds.length !== expectedComponentCount || m.componentIds.length !== m.components.length || new Set(m.componentIds).size !== m.componentIds.length || new Set(m.components.map(c => c.id)).size !== m.components.length || m.components.some(c => !text(c.id) || !atoms(c.valueAtoms) || c.valueAtoms > UINT256_MAX || !Number.isSafeInteger(c.scale) || c.scale < 0 || c.scale > 36 || c.quoteCurrency !== currency || !m.componentIds!.includes(c.id)))) throw new Error("M5_MARKET_LIQUIDITY_COVERAGE_INVALID");
}
...
export function createM5MarketMetricsAuthority(input: Readonly<{ providerId: string; datasetId: string; datasetVersion: string; sourceLineageId: string; asOf: string; quoteCurrency: string; materials: readonly MarketSourceMaterial[]; recordedAt: string }>): M5MarketMetricsResult {
  ...
  if (!liquidityMaterial?.components || !liquidityMaterial.componentIds || liquidityMaterial.coverageComplete !== true || liquidityMaterial.expectedComponentCount === undefined || liquidityMaterial.expectedComponentCount < 2 || liquidityMaterial.componentIds.length !== liquidityMaterial.expectedComponentCount) return incomplete("M5_MARKET_LIQUIDITY_COVERAGE_INCOMPLETE");
  ...
  const sourceIds = input.materials.flatMap(m => [m.sourceArtifactId, m.sourceEnvelopeId, m.sourceObservationId, m.providerExternalRecordId]);
  if (new Set(sourceIds).size !== sourceIds.length) return invalid("M5_MARKET_MATERIAL_DUPLICATE");
  const liquidityComponents: readonly MarketLiquidityComponent[] | undefined = liquidityMaterial.components;
  const liquidityIds: readonly string[] | undefined = liquidityMaterial.componentIds;
  const expectedComponentCount = liquidityMaterial.expectedComponentCount;
  if (!liquidityComponents || !liquidityIds || liquidityMaterial.coverageComplete !== true || expectedComponentCount === undefined || !Number.isSafeInteger(expectedComponentCount) || expectedComponentCount < 2 || liquidityIds.length !== expectedComponentCount || liquidityIds.length !== liquidityComponents.length) return incomplete("M5_MARKET_LIQUIDITY_COVERAGE_INCOMPLETE");
  const normalizedLiquidity = liquidityComponents.map((component: MarketLiquidityComponent, index: number) => ({ component, id: liquidityIds[index]! })).sort((a: { id: string; component: MarketLiquidityComponent }, b: { id: string; component: MarketLiquidityComponent }) => a.id.localeCompare(b.id));
  if (normalizedLiquidity.some(entry => entry.component.id !== entry.id)) return invalid("M5_MARKET_LIQUIDITY_COMPONENT_ORDER_INVALID");
  const normalizedMaterials = input.materials.map(m => m.metricKind === "LIQUIDITY" ? { ...m, componentIds: normalizedLiquidity.map(entry => entry.id), components: normalizedLiquidity.map(entry => entry.component) } : m);
  const valueFor = (kind: MarketMetricKind): bigint => { const m = normalizedMaterials.find(item => item.metricKind === kind)!; return kind === "LIQUIDITY" ? m.components!.reduce((sum: bigint, c: Readonly<{ valueAtoms: bigint; scale: number }>) => sum + minor(c.valueAtoms, c.scale), 0n) : minor(m.valueAtoms, m.scale); };
  const values = expected.map(valueFor);
  if (values.some(value => value > UINT256_MAX)) return invalid("M5_MARKET_VALUE_RANGE_INVALID");
  const liquidity = normalizedMaterials.find(m => m.metricKind === "LIQUIDITY")!;
  ...
  const derivations = expected.map((kind, i) => {
    const m = normalizedMaterials.find(x => x.metricKind === kind)!;
    const body = { contractVersion: M5_MARKET_METRICS_AUTHORITY_VERSION, metricKind: kind, valueAtoms: values[i]!.toString(), unit: "MINOR", scale: 0, quoteCurrency: input.quoteCurrency, basis: m.basis, windowStart: m.windowStart, windowEnd: m.windowEnd, coverageVersion: m.coverageVersion, coverageComplete: m.coverageComplete, expectedComponentCount: m.expectedComponentCount, componentIds: m.componentIds, components: m.components?.map((c: MarketLiquidityComponent) => ({ id: c.id, valueAtoms: c.valueAtoms.toString(), scale: c.scale, quoteCurrency: c.quoteCurrency })), sourceValueAtoms: m.valueAtoms.toString(), sourceScale: m.scale, sourceQuoteCurrency: m.quoteCurrency, materialIds: [m.sourceArtifactId, m.sourceEnvelopeId, m.sourceObservationId, m.providerExternalRecordId], materialFingerprints: [m.payloadFingerprint], observedAt: m.observedAt, availableAt: m.availableAt, asOf: input.asOf };
