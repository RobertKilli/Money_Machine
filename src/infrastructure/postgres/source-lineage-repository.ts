...
function createRepository(client: TransactionSql): SourceLineageRepository {
  return {
    createFromClaims: async (scope, claimIds, recordedAt) => {
      ...
      const inserted = await client`insert into public.intelligence_source_lineages (contract_version,source_lineage_id,provider_id,dataset_id,dataset_version,availability_claim_ids,source_artifact_ids,ingestion_attempt_ids,member_count,observed_at,effective_available_at,fingerprint,recorded_at) values (${built.lineage.contractVersion},${built.lineage.sourceLineageId},${built.lineage.providerId},${built.lineage.datasetId},${built.lineage.datasetVersion},${client.json(built.lineage.availabilityClaimIds)},${client.json(built.lineage.sourceArtifactIds)},${client.json(built.lineage.ingestionAttemptIds)},${built.lineage.memberCount},${built.lineage.observedAt},${built.lineage.effectiveAvailableAt},${built.lineage.fingerprint},${built.lineage.recordedAt}) on conflict (source_lineage_id) do nothing returning source_lineage_id`;
    ...
    validateForMappingCreation: async sourceLineageId => {
      const parent = await (async () => {
        ...
        if (rows.length !== 1) throw new Error("M5_MAPPING_SOURCE_LINEAGE_NOT_FOUND");
        return mapSourceLineageRow(row(rows[0]));
      })();
      ...
      validateMembers(parent, members);
      const attemptIds = [...parent.ingestionAttemptIds].sort((a, b) => a.localeCompare(b));
      for (const attemptId of attemptIds) {
        ...
        if (locked.length !== 1) throw new Error("M5_MAPPING_SOURCE_LINEAGE_ATTEMPT_INVALID");
        ...
        const status = reduceIngestionLifecycle(Object.freeze(events.map(value => mapLifecycleEventRow(row(value))))).status;
        if (status === "PARTIAL") throw new Error("M5_MAPPING_SOURCE_LINEAGE_ATTEMPT_PARTIAL");
        if (status === "FAILED") throw new Error("M5_MAPPING_SOURCE_LINEAGE_ATTEMPT_FAILED");
        if (status === "CANCELLED") throw new Error("M5_MAPPING_SOURCE_LINEAGE_ATTEMPT_CANCELLED");
        if (status !== "COMPLETED") throw new Error("M5_MAPPING_SOURCE_LINEAGE_ATTEMPT_OPEN");
      }
      return parent;
    },
