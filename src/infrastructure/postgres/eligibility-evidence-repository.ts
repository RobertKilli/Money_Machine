...
export function mapQuantitativeEligibilityEvidenceRow(row: RawRow) {
  ...
  return verified(
    createQuantitativeEligibilityEvidence({
      ...
      dailySeriesAuthorityId: optionalText(row.daily_series_authority_id),
      dailySeriesAuthorityFingerprint: optionalText(row.daily_series_authority_fingerprint),
      dailySeriesDerivationFingerprint: optionalText(row.daily_series_derivation_fingerprint),
      marketMetricsAuthorityId: optionalText(row.market_metrics_authority_id),
      marketMetricsAuthorityFingerprint: optionalText(row.market_metrics_authority_fingerprint),
      marketMetricsDerivationFingerprint: optionalText(row.market_metrics_derivation_fingerprint),
...
export function mapVenueEligibilityEvidenceRow(row: RawRow) {
  ...
  return verified(
    createVenueEligibilityEvidence({
      ...
      ...(row.venue_authority_id == null ? {} : { venueAuthorityId: text(row.venue_authority_id, "M5_RAW_ROW_VENUE_AUTHORITY_INVALID"), venueAuthorityFingerprint: text(row.venue_authority_fingerprint, "M5_RAW_ROW_VENUE_AUTHORITY_INVALID"), venueMemberId: text(row.venue_member_id, "M5_RAW_ROW_VENUE_AUTHORITY_INVALID"), venueMemberFingerprint: text(row.venue_member_fingerprint, "M5_RAW_ROW_VENUE_AUTHORITY_INVALID"), venueAuthorityMemberCount: Number(row.venue_authority_member_count), asOf: canonicalTimestamp(timestamp(row.as_of, "M5_RAW_ROW_VENUE_AS_OF_INVALID"), "M5_RAW_ROW_VENUE_AS_OF_INVALID") }),
    } as never),
    row.fingerprint,
  );
}
...
async function insert(
  ...
  result =
      await client`insert into public.eligibility_quantitative_evidence (evidence_id,candidate_id,asset_id,canonical_identifier,asset_class,provider_id,dataset_id,dataset_version,mapping_revision_id,source_lineage_id,observed_at,available_at,provenance,fingerprint,metric_kind,value_atoms,scale,unit,semantics_version,currency_code,window_start_at,window_end_at,qualification_basis,holder_snapshot_id,holder_snapshot_fingerprint,holder_derivation_fingerprint,daily_series_authority_id,daily_series_authority_fingerprint,daily_series_derivation_fingerprint,market_metrics_authority_id,market_metrics_authority_fingerprint,market_metrics_derivation_fingerprint,as_of) values (${record.evidenceId},${record.candidateId},${record.assetId},${record.canonicalIdentifier},${record.assetClass},${record.providerId},${record.datasetId},${record.datasetVersion},${record.mappingRevisionId},${record.sourceLineageId},${record.observedAt},${record.availableAt},${jsonParameter(client, record.provenance)},${record.fingerprint},${record.metricKind},${record.valueAtoms.toString()}::numeric,${record.scale},${record.unit},${record.semanticsVersion},${record.currencyCode ?? null},${record.window?.startAt ?? null},${record.window?.endAt ?? null},${record.qualificationBasis ?? null},${record.holderSnapshotId ?? null},${record.holderSnapshotFingerprint ?? null},${record.holderDerivationFingerprint ?? null},${record.dailySeriesAuthorityId ?? null},${record.dailySeriesAuthorityFingerprint ?? null},${record.dailySeriesDerivationFingerprint ?? null},${record.marketMetricsAuthorityId ?? null},${record.marketMetricsAuthorityFingerprint ?? null},${record.marketMetricsDerivationFingerprint ?? null},${record.asOf ?? null}) on conflict (evidence_id) do nothing returning evidence_id`;
  else if (record.evidenceKind === "VENUE")
    result =
      await client`insert into public.eligibility_venue_evidence (evidence_id,candidate_id,asset_id,canonical_identifier,asset_class,provider_id,dataset_id,dataset_version,mapping_revision_id,source_lineage_id,observed_at,available_at,provenance,fingerprint,venue_id,eligibility_state,venue_authority_id,venue_authority_fingerprint,venue_member_id,venue_member_fingerprint,venue_authority_member_count,as_of) values (${record.evidenceId},${record.candidateId},${record.assetId},${record.canonicalIdentifier},${record.assetClass},${record.providerId},${record.datasetId},${record.datasetVersion},${record.mappingRevisionId},${record.sourceLineageId},${record.observedAt},${record.availableAt},${jsonParameter(client, record.provenance)},${record.fingerprint},${record.venueId},${record.eligibilityState},${record.venueAuthorityId ?? null},${record.venueAuthorityFingerprint ?? null},${record.venueMemberId ?? null},${record.venueMemberFingerprint ?? null},${record.venueAuthorityMemberCount ?? null},${record.asOf ?? null}) on conflict (evidence_id) do nothing returning evidence_id`;
