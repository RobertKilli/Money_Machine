...
export function mapIngestionRequestRow(value: RawRow): IngestionRequest {
  return createIngestionRequest({
    ...
    contractVersion: text(value.contract_version, "M5_INGESTION_REQUEST_ROW_CONTRACT_INVALID") as IngestionRequest["contractVersion"],
    ...
    requestFingerprint: text(value.request_fingerprint, "M5_INGESTION_REQUEST_ROW_FINGERPRINT_INVALID"),
...
export function mapIngestionAttemptRow(value: RawRow): IngestionAttempt {
  return createIngestionAttempt({
    ingestionAttemptId: text(value.ingestion_attempt_id, "M5_INGESTION_ATTEMPT_ROW_ID_INVALID"),
    contractVersion: text(value.contract_version, "M5_INGESTION_ATTEMPT_ROW_CONTRACT_INVALID") as IngestionAttempt["contractVersion"],
    ...
    attemptFingerprint: text(value.attempt_fingerprint, "M5_INGESTION_ATTEMPT_ROW_FINGERPRINT_INVALID"),
...
export function mapLifecycleEventRow(value: RawRow): LifecycleEvent {
  ...
  return createLifecycleEvent({
    lifecycleEventId: text(value.lifecycle_event_id, "M5_INGESTION_EVENT_ROW_ID_INVALID"),
    ingestionAttemptId: text(value.ingestion_attempt_id, "M5_INGESTION_EVENT_ROW_ATTEMPT_INVALID"),
    contractVersion: text(value.contract_version, "M5_INGESTION_EVENT_ROW_CONTRACT_INVALID") as LifecycleEvent["contractVersion"],
    ...
    eventFingerprint: text(value.event_fingerprint, "M5_INGESTION_EVENT_ROW_FINGERPRINT_INVALID"),
...
export function mapSourceArtifactRow(value: RawRow): SourceArtifact {
  return createSourceArtifact({
    sourceArtifactId: text(value.source_artifact_id, "M5_SOURCE_ARTIFACT_ROW_ID_INVALID"),
    contractVersion: text(value.contract_version, "M5_SOURCE_ARTIFACT_ROW_CONTRACT_INVALID") as SourceArtifact["contractVersion"],
    ...
    sourceArtifactFingerprint: text(value.source_artifact_fingerprint, "M5_SOURCE_ARTIFACT_ROW_FINGERPRINT_INVALID"),
...
export function mapSourceEnvelopeRow(value: RawRow): SourceEnvelope {
  return createSourceEnvelope({
    sourceEnvelopeId: text(value.source_envelope_id, "M5_SOURCE_ENVELOPE_ROW_ID_INVALID"),
    contractVersion: text(value.contract_version, "M5_SOURCE_ENVELOPE_ROW_CONTRACT_INVALID") as SourceEnvelope["contractVersion"],
    ...
    sourceEnvelopeFingerprint: text(value.source_envelope_fingerprint, "M5_SOURCE_ENVELOPE_ROW_FINGERPRINT_INVALID"),
...
export function mapSourceObservationRow(value: RawRow): SourceObservation {
  return createSourceObservation({
    sourceObservationId: text(value.source_observation_id, "M5_SOURCE_OBSERVATION_ROW_ID_INVALID"),
    contractVersion: text(value.contract_version, "M5_SOURCE_OBSERVATION_ROW_CONTRACT_INVALID") as SourceObservation["contractVersion"],
    ...
    observationFingerprint: text(value.observation_fingerprint, "M5_SOURCE_OBSERVATION_ROW_FINGERPRINT_INVALID"),
...
export function mapAvailabilityClaimRow(value: RawRow): AvailabilityClaim {
  const record: AvailabilityClaim = {
    availabilityClaimId: text(value.availability_claim_id, "M5_AVAILABILITY_ROW_ID_INVALID"),
    ...
    contractVersion: text(value.contract_version, "M5_AVAILABILITY_ROW_CONTRACT_INVALID") as AvailabilityClaim["contractVersion"],
    ...
    claimFingerprint: text(value.claim_fingerprint, "M5_AVAILABILITY_ROW_FINGERPRINT_INVALID"),
    recordedAt: timestamp(value.recorded_at, "M5_AVAILABILITY_ROW_RECORDED_INVALID"),
  };
  assertAvailabilityClaim(record);
  return record;
}
...
async function saveRequest(client: DbClient, input: IngestionRequest): Promise<IngestionRequest> {
  ...
  if (inserted.length) return record;
...
export function createIngestionProvenanceRepositories(client: DbClient): AsyncIngestionProvenanceRepositories {
...
export function createIngestionProvenanceUnitOfWork(client: Sql): IngestionProvenanceUnitOfWork {
  return { withTransaction: <T>(work: (repositories: AsyncIngestionProvenanceRepositories) => Promise<T>) => client.begin(async transaction => work(createIngestionProvenanceRepositories(transaction))) as unknown as Promise<T> };
}
...
export async function withIngestionProvenanceUnitOfWork<T>(work: (repositories: AsyncIngestionProvenanceRepositories) => Promise<T>): Promise<T> {
  ...
  try { return await sql.begin(async transaction => work(createIngestionProvenanceRepositories(transaction))) as unknown as T; } finally { await sql.end({ timeout: 5 }); }
}
