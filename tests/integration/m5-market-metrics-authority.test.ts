...
function packageFixture() {
  const records = [
    ...
    { id: "market:volume", kind: "VOLUME", value: (750_000n * huge + 456n).toString(), basis: "ROLLING_24H_REPORTED", windowStart: "2026-02-01T00:00:00.000Z", windowEnd: "2026-02-02T00:00:00.000Z" },
    ...
    const envelope = { provider: "synthetic-market", metricKind: item.kind, valueAtoms: item.value, sourceScale: 18, quoteCurrency: "USD", basis: item.basis, observedAt, ...(item.windowStart ? { windowStart: item.windowStart, windowEnd: item.windowEnd } : {}), ...(item.coverageVersion ? { coverageVersion: item.coverageVersion, coverageComplete: true, expectedComponentCount: 2, componentAId: "pool-a", componentAAtoms: (750_000n * huge).toString(), componentBId: "pool-b", componentBAtoms: (500_000n * huge).toString(), componentScale: 18 } : {}) };
