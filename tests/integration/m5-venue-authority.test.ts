...
describe.skipIf(!enabled)("M5 venue authority PostgreSQL integration", () => {
  it("persists exact venue set and atomically binds evidence", async () => {
    ...
    try {
      ...
      const rollbackUow: typeof baseEvidenceUow = { withTransaction: work => baseEvidenceUow.withTransaction(async repositories => { let writes = 0; return work({ ...repositories, evidence: { save: async row => { const saved = await repositories.evidence.save(row); writes += 1; if (writes === 1) throw new Error("M5_TEST_VENUE_ROLLBACK_SENTINEL"); return saved; } } }); }) };
