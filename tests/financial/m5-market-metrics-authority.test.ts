...
const base = { payloadFingerprint: sha, scale: 2, quoteCurrency: "USD", observedAt: "2026-01-31T23:00:00.000Z", availableAt: "2026-01-31T23:30:00.000Z" };
const materials = [
  { ...base, metricKind: "MARKET_CAP" as const, sourceArtifactId: "artifact-cap", sourceEnvelopeId: "envelope-cap", sourceObservationId: "observation-cap", providerExternalRecordId: "provider-cap", valueAtoms: 15_000_000n, basis: "MARKET_CAP_REPORTED" },
  { ...base, metricKind: "VOLUME" as const, sourceArtifactId: "artifact-volume", sourceEnvelopeId: "envelope-volume", sourceObservationId: "observation-volume", providerExternalRecordId: "provider-volume", valueAtoms: 750_000n, basis: "ROLLING_24H_REPORTED", windowStart: "2026-01-31T00:00:00.000Z", windowEnd: "2026-02-01T00:00:00.000Z" },
  { ...base, metricKind: "LIQUIDITY" as const, sourceArtifactId: "artifact-liquidity", sourceEnvelopeId: "envelope-liquidity", sourceObservationId: "observation-liquidity", providerExternalRecordId: "provider-liquidity", valueAtoms: 1_250_000n, basis: "COMPLETE_LIQUIDITY_UNIVERSE", coverageVersion: "universe/v1", coverageComplete: true, expectedComponentCount: 2, componentIds: ["pool-a", "pool-b"], components: [{ id: "pool-a", valueAtoms: 750_000n, scale: 2, quoteCurrency: "USD" }, { id: "pool-b", valueAtoms: 500_000n, scale: 2, quoteCurrency: "USD" }] },
];
...
describe("M5 market metrics authority", () => {
  ...
  it("changes material fingerprints when a liquidity component changes", () => {
    const changed = materials.map(m => m.metricKind === "LIQUIDITY" ? { ...m, valueAtoms: m.valueAtoms + 1n, components: m.components!.map(c => c.id === "pool-b" ? { ...c, valueAtoms: c.valueAtoms + 1n } : c) } : m);
    const original = createM5MarketMetricsAuthority({ providerId: "p", datasetId: "d", datasetVersion: "v", sourceLineageId: "l", asOf, quoteCurrency: "USD", materials, recordedAt: asOf });
    const mutated = createM5MarketMetricsAuthority({ providerId: "p", datasetId: "d", datasetVersion: "v", sourceLineageId: "l", asOf, quoteCurrency: "USD", materials: changed, recordedAt: asOf });
    expect(original.status).toBe("READY"); expect(mutated.status).toBe("READY");
    if (original.status === "READY" && mutated.status === "READY") expect(mutated.authority.fingerprint).not.toBe(original.authority.fingerprint);
  });
});
