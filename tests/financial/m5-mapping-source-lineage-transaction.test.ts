...
const value = (overrides: Record<string, unknown> = {}) => ({ sourceLineageId: "lineage-1", providerAssetIdentityAssertionId: "assertion-1", canonicalAssetId: "canonical", canonicalIdentifier: "canonical:asset", assetClass: "CRYPTO", mappingRevisionVersion: "mapping/v1", validFrom: "2026-01-01T00:00:00.000Z", recordedAt: "2026-01-01T00:02:00.000Z", ...overrides });
const execute = async (lineageOverrides: Record<string, unknown> = {}, valueOverrides: Record<string, unknown> = {}, validator?: () => Promise<never>) => {
  ...
  const providerAssetIdentityAssertionRepository = { readById: async () => ({ providerAssetIdentityAssertionId: "assertion-1", providerId: "p", datasetId: "d", datasetVersion: "v", providerSourceNamespace: "CHAIN:TEST", providerAssetId: "asset", sourceArtifactId: "artifact", sourceEnvelopeId: "envelope" }) };
