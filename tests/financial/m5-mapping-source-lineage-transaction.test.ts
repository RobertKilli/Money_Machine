...
const value = (overrides: Record<string, unknown> = {}) => ({ sourceLineageId: "lineage-1", providerId: "p", datasetId: "d", datasetVersion: "v", providerSourceNamespace: "CHAIN:TEST", providerAssetId: "asset", canonicalAssetId: "canonical", canonicalIdentifier: "canonical:asset", assetClass: "CRYPTO", mappingRevisionVersion: "mapping/v1", validFrom: "2026-01-01T00:00:00.000Z", recordedAt: "2026-01-01T00:02:00.000Z", ...overrides });
const execute = async (lineageOverrides: Record<string, unknown> = {}, valueOverrides: Record<string, unknown> = {}, validator?: () => Promise<never>) => {
  const saves: unknown[] = [];
  const sourceLineageRepository = { readById: async () => lineage(lineageOverrides), readMembers: async () => [], createFromClaims: async () => lineage(), ...(validator ? { validateForMappingCreation: validator } : {}) };
  const mappingRepository = { save: async (mapping: unknown) => { saves.push(mapping); return mapping; }, readCandidatesAt: async () => [] };
  let transactions = 0;
  const unitOfWork = { withTransaction: async <T>(work: (repositories: { sourceLineageRepository: typeof sourceLineageRepository; mappingRepository: typeof mappingRepository }) => Promise<T>): Promise<T> => { transactions += 1; return work({ sourceLineageRepository, mappingRepository }); } };
