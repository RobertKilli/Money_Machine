...
const complete = (overrides: { readonly liquidity?: bigint } = {}) => {
  ...
  const venueA = createVenueEligibilityEvidence(base({ evidenceId: "venue-a", venueId: "m5-venue-member-a", eligibilityState: "ELIGIBLE", venueAuthorityId: "venue-authority-1", venueAuthorityFingerprint: "b".repeat(64), venueMemberId: "m5-venue-member-a", venueMemberFingerprint: "c".repeat(64), venueAuthorityMemberCount: 2, asOf }) as never);
  const venueB = createVenueEligibilityEvidence(base({ evidenceId: "venue-b", venueId: "m5-venue-member-b", eligibilityState: "ELIGIBLE", venueAuthorityId: "venue-authority-1", venueAuthorityFingerprint: "b".repeat(64), venueMemberId: "m5-venue-member-b", venueMemberFingerprint: "d".repeat(64), venueAuthorityMemberCount: 2, asOf }) as never);
  const raw = [age, history, liquidity, volume, marketCap, top10, single, volatility, venueA, venueB] as const;
  const assessment = createM5SuspiciousAssessment({ contractVersion: "m5-suspicious-assessment/v1", providerId: "provider", datasetId: "dataset", datasetVersion: "v1", candidateId: "candidate", assetId: "asset", canonicalIdentifier: "asset:one", assetClass: "CRYPTO", mappingRevisionId: "mapping-1", sourceLineageId: "lineage-1", ruleSetVersion: "rules/v1", ruleSetFingerprint: trustedRuleSet.fingerprint, ruleSetAuthorityId: trustedRuleSet.ruleSetAuthorityId, coverageAuthorityId: "coverage-authority", coverageFingerprint: "c".repeat(64), evaluatedRuleCount: 1, coverageStatus: "COMPLETE", detectorVersion: "detector/v1", coveredRuleIds: ["RULE_A"], result: "NO_FINDINGS", findingReferences: [], asOf, observedAt: "2026-09-10T00:00:00.000Z", availableAt: "2026-09-12T00:00:00.000Z", sourceRecordIds: ["source"], payloadFingerprint: "a".repeat(64), datasetPins: [encodeM5DatasetPin(pin)], recordedAt: asOf });
...
describe("M5 producer orchestration", () => {
  ...
  it("maps all four repository evidence families and rejects stored fingerprint conflicts", () => {
    ...
    expect(mapVenueEligibilityEvidenceRow({ ...row(venue), venue_id: venue.venueId, eligibility_state: "ELIGIBLE" })).toBeTruthy();
