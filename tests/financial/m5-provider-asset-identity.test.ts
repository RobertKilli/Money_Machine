...
import { mapProviderAssetIdentityAssertionRow } from "@/infrastructure/postgres/provider-asset-identity-repository";
...
const row = (value: ReturnType<typeof createProviderAssetIdentityAssertion>) => ({ provider_asset_identity_assertion_id: value.providerAssetIdentityAssertionId, contract_version: value.contractVersion, provider_id: value.providerId, dataset_id: value.datasetId, dataset_version: value.datasetVersion, provider_source_namespace: value.providerSourceNamespace, provider_asset_id: value.providerAssetId, identity_type: value.identityType, identity_namespace: value.identityNamespace, identity_value: value.identityValue, source_artifact_id: value.sourceArtifactId, source_envelope_id: value.sourceEnvelopeId, parser_version: value.parserVersion, envelope_schema_version: value.envelopeSchemaVersion, source_payload_fingerprint: value.sourcePayloadFingerprint, fingerprint: value.fingerprint, recorded_at: value.recordedAt });
...
describe("M5 provider asset identity foundation", () => {
  it("normalizes EVM addresses, binds parser/schema in ID, excludes recordedAt, and freezes output", () => {
    ...
    expect(first.identityValue).toBe("0xabcdef0123456789abcdef0123456789abcdef01"); expect(first.providerAssetIdentityAssertionId).toBe(replay.providerAssetIdentityAssertionId); expect(first.fingerprint).toBe(replay.fingerprint); expect(createProviderAssetIdentityAssertion(input({ parserVersion: "parser/v2" })).providerAssetIdentityAssertionId).not.toBe(first.providerAssetIdentityAssertionId); expect(createProviderAssetIdentityAssertion(input({ sourcePayloadFingerprint: "b".repeat(64) })).providerAssetIdentityAssertionId).toBe(first.providerAssetIdentityAssertionId); expect(createProviderAssetIdentityAssertion(input({ sourcePayloadFingerprint: "b".repeat(64) })).fingerprint).not.toBe(first.fingerprint); expect(Object.isFrozen(first)).toBe(true);
  });
  ...
  it("reconstructs stored rows and fails closed on contract, ID, and fingerprint corruption", () => {
    const value = createProviderAssetIdentityAssertion(input());
    expect(mapProviderAssetIdentityAssertionRow(row(value))).toEqual(value);
    expect(() => mapProviderAssetIdentityAssertionRow({ ...row(value), contract_version: "wrong/v1" })).toThrow("M5_PROVIDER_ASSET_IDENTITY_ROW_CONTRACT_INVALID");
    expect(() => mapProviderAssetIdentityAssertionRow({ ...row(value), provider_asset_identity_assertion_id: "forged" })).toThrow("M5_PROVIDER_ASSET_IDENTITY_ID_MISMATCH");
    expect(() => mapProviderAssetIdentityAssertionRow({ ...row(value), fingerprint: "b".repeat(64) })).toThrow("M5_PROVIDER_ASSET_IDENTITY_FINGERPRINT_MISMATCH");
  });
  it("replays the same assertion ID and rejects same-ID different-material conflicts", () => {
    const first = createProviderAssetIdentityAssertion(input());
    const changed = createProviderAssetIdentityAssertion(input({ sourcePayloadFingerprint: "b".repeat(64) }));
    expect(changed.providerAssetIdentityAssertionId).toBe(first.providerAssetIdentityAssertionId);
