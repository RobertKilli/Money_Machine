...
describe("M5 provider live acquisition", () => {
  it("creates only deterministic, fixed-host CoinGecko and Etherscan request plans", () => {
    ...
    expect(coin.requests[0]).toMatchObject({ protocol: "https:", hostname: "pro-api.coingecko.com", path: "/api/v3/coins/ethereum/market_chart/range" });
  ...
  it("parses live CoinGecko bytes through the fixture contract with receipt-independent payload identity", () => {
    const response = body('{"prices":[[1767225600000,1e-8]],"market_caps":[[1767225600000,1234.5]],"total_volumes":[[1767225600000,0]]}');
  ...
  it("keeps production execution blocked before credential and transport ports", async () => {
    ...
    const result = await executeM5ProviderLiveAcquisition({
      aggregate, readiness, request: coinRequest(), asOf: now, credential: { kind: "API_KEY", reference: "env:coingecko-pro-api-key" },
      credentials: { resolve: credentialResolve }, transport: { send } as M5ProviderHttpTransport,
      rateLimit: { acquire: vi.fn(async () => true) }, idempotencyKey: "blocked", requestedAt: now, startedAt: now, recordedAt: now,
    });
    ...
    const forged = await executeM5ProviderLiveAcquisition({ aggregate: handBuilt, readiness, request: coinRequest(), asOf: now,
      ...
      idempotencyKey: "forged", requestedAt: now, startedAt: now, recordedAt: now });
    expect(forged.status).toBe("BLOCKED");
    expect(credentialResolve).not.toHaveBeenCalled();
    expect(send).not.toHaveBeenCalled();
  });

  it("rejects noncanonical asOf before either credential or HTTP boundary", async () => {
    const { readiness, aggregate } = productionAggregate();
    const credentialResolve = vi.fn(async () => ({ kind: "API_KEY" as const, value: "never" }));
    const send = vi.fn();
    const result = await executeM5ProviderLiveAcquisition({ aggregate, readiness, request: coinRequest(), asOf: "2026-09-25T12:00:00Z",
      credential: { kind: "API_KEY", reference: "env:coingecko-pro-api-key" }, credentials: { resolve: credentialResolve },
      transport: { send } as unknown as M5ProviderHttpTransport, rateLimit: { acquire: async () => true },
      idempotencyKey: "invalid-time", requestedAt: now, startedAt: now, recordedAt: now });
    expect(result).toEqual({ status: "INVALID", code: "M5_PROVIDER_LIVE_AS_OF_INVALID" });
    expect(credentialResolve).not.toHaveBeenCalled();
    expect(send).not.toHaveBeenCalled();
  });
});
